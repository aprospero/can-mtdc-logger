...
* 'strict aliasing' compiler optimization needs to be disabled (eg. -fno-strict-aliasing for GCC)
...
Incoming CAN bus messages from eg. Sorel MTDC/LTDC devices are assumed to be fed to Sorella™ by calling scbi_parse() for each incoming frame. 
If an incoming scbi frame reports value changes of registered parameters, those are stored in a parameter queue. Duplicate parameter changes are filtered over the time of a configurable timeout. After each call of scbi_parse the function scbi_pop_param needs to be called until it doesn't provide any more parameter changes.
...
counting to [**SCBI_TIME_MAX**](#SCBI_TIME_MAX) = 2³² = 4294967296 ms, scbi_time will overflow to zero after 49 days, 17 hours, 2 min, 47 sec and 296 ms.
...
This data structure holds a complete CAN frame.
...
* [scbi_time](#scbi_time() **recvd** - timestamp holding time of dispatch/reception
...
SCBI datalogger monitor parameter types

The datalogger monitor program msg type addresses parameters from several different types of devices. 

###### Members

* **SCBI_PARAM_TYPE_SENSOR**   sensory parameters - there are [SCBI_MAX_SENSORS ](#SCBI_MAX_SENSORS--SCBI_MAX_RELAYS) addressable.

*  **SCBI_PARAM_TYPE_RELAY**   relay parameters - there are [SCBI_MAX_RELAYS](#SCBI_MAX_SENSORS--SCBI_MAX_RELAYS) addressable.

* **SCBI_PARAM_TYPE_OVERVIEW**   overview parameters - these are mainly parameters with statistical yield data.
...
enum scbi_param_type
{
  ...
  SCBI_PARAM_TYPE_COUNT,    /* used for range validation */
  SCBI_PARAM_TYPE_NONE      /* invalid - don't use */
};
```
---
#### struct scbi_param

Contains data for a parameter of the MTDC/LTDC. This public API structure is the main output generated by Sorella™ library to signal a parameters value change.
